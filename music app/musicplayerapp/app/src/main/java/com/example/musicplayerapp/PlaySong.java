...
public class PlaySong extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Toggle repeat on tap
        repeat.setOnClickListener(v -> {
            isRepeatEnabled = !isRepeatEnabled;
            repeatCount = 0; // reset count whenever toggled
            // visually indicate state (e.g. change tint or alpha)
            repeat.setAlpha(isRepeatEnabled ? 1.0f : 0.4f);
            Toast.makeText(this,
                    isRepeatEnabled ? "Repeat ×5 ON" : "Repeat OFF",
                    Toast.LENGTH_SHORT).show();
        });

        // Listen for completion
        mediaPlayer.setOnCompletionListener(mp -> {
            if (isRepeatEnabled && repeatCount < MAX_REPEAT - 1) {
                repeatCount++;
                mp.seekTo(0);
                mp.start();
            } else {
                // either repeat off or max reached:
                // reset state if you like
                repeatCount = 0;
